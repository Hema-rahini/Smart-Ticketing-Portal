...
export function DepartmentPerformanceChart() {
  ...
  // Derive unique departments from users and tickets
  const uniqueDepartments = Array.from(
    new Set([
      ...users.map(u => u.department).filter(Boolean),
      ...tickets.map(t => t.department).filter(Boolean)
    ])
  ) as string[]

  const departmentData = uniqueDepartments.map(deptName => ({
    name: deptName,
    tickets: tickets.filter(t => t.department === deptName).length,
    completed: tickets.filter(t => t.department === deptName && t.status === 'completed').length,
