...
export function TopNavbar({ title }: TopNavbarProps) {
  ...
  const getUser = (userId: string) => useStore.getState().users.find(u => u.id === userId)

  // Filter notifications relevant strictly to current user
  const userNotifications = activities.filter(activity => {
    ...
    const sender = getUser(activity.userId)
    if (currentUser?.role === 'admin') return true
    if (sender?.department && currentUser?.department && sender.department === currentUser.department) return true
                ...
                recentNotifications.map((activity) => {
                  ...
                  return (
                    <DropdownMenuItem key={activity.id} className="flex flex-col items-start gap-1 py-3 cursor-pointer">
                      <p className="text-sm">
                        <span className="font-semibold">{sender?.name || 'System'}</span>{' '}
